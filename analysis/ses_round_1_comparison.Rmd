...
completion_times_round_1 <- 
  rbind(
    ses_round_1_wg %>% 
      select(study_name, subject_id, completion_time, age) %>% 
...
summary(completion_times_round_1)
View(completion_times_round_1 %>% arrange(completion_time))
...
Well, that looks like a problem...let's look into ages...

```{r}

completion_times_round_1 %>% 
  ggplot(aes(age, completion_time)) +
...
Still looks weird. Let's look into the last experiment...

```{r}

View(ses_pilot_raw %>% getCompletionInterval() %>% arrange(completion_time))
...
Significantly less bad, which is weird. Let's look into demographics for the newer set.
